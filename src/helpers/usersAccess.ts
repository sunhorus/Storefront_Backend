...
export default class UsersStoreAccess implements UsersStore {
    ...
    async create(user: User): Promise<User> {
        try {
            const conn = await PostgresClient.connect();
            const query = 'INSERT INTO users (username, firstName, lastName, password_digest) VALUES ($1, $2, $3, $4) RETURNING *';
            const hash = bcrypt.hashSync(user.password_digest + pepper, saltRounds);
            const result = await conn.query(query, [user.username, user.firstName, user.lastName, hash]);
    ...
    async authenticate(username: string, Password: string): Promise<User | null> {
        try {
            ...
            if (result.rows.length) {
                ...
                console.log(result.rows[0])
                if (bcrypt.compareSync(Password + pepper, uPassDigist.password_digest)) {
